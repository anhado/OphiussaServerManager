...
namespace OphiussaServerManager.Common.Ini {
    ...
    public class FloatIniValueArray : IniValueList<float>, IIniValuesList
    {
        ...
        {
            return val.ToString("0.0#########", CultureInfo.GetCultureInfo(DEFAULT_CULTURE_CODE));
        }

        private static float FromIniValueInternal(string iniVal)
        {
            var tempValue = iniVal.Replace("f", "");
            return float.Parse(tempValue, NumberStyles.AllowDecimalPoint | NumberStyles.AllowLeadingSign, CultureInfo.GetCultureInfo(DEFAULT_CULTURE_CODE));
        }
