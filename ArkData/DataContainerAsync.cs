...
namespace ArkData {
    ...
    public partial class DataContainer {
        ...
        public static async Task<DataContainer> CreateAsync(string playerFileFolder, string tribeFileFolder) {
            ...
            if (tribeFiles != null)
                foreach (string file in tribeFiles)
        ...
        public async Task<DateTime> LoadSteamAsync(string apiKey, int steamUpdateInterval = 0) {
            ...
            int startIndex         = 0;
            string[] playerSteamIds = Players
                                     .Where(p => p.LastPlatformUpdateUtc.AddMinutes(steamUpdateInterval) < DateTime.UtcNow && p.NoUpdateCount < MaxInvalidCount)
                                     .Select(p => p.PlayerId)
                                     .ToArray();
            ...
            while (true) {
                ...
                int steamIdsCount = Math.Min(MaxSteamIds, playerSteamIds.Count() - startIndex);
                // get a comma delimited list of the steam ids to process
                string builder = string.Join(",", playerSteamIds, startIndex, steamIdsCount);
        ...
        public Task LoadOnlinePlayersAsync(string ipString, int port) {
            if (SteamLoaded)
                return Task.Run(() => { LinkOnlinePlayers(ipString, port); });
