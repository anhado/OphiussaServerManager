...
namespace OphiussaServerManager
{
    public static class SteamUtils
    {
        ...
        public static PublishedFileDetailsResponse GetSteamModDetails(List<string> modIdList) => SteamUtils.GetSteamModDetails(new List<(string, List<string>)>()
    {
      ("", modIdList)
    });

        public static PublishedFileDetailsResponse GetSteamModDetails(
                ...
                foreach ((string AppId, List<string> ModIdList) appMod in appMods)
                {
                    if (appMod.ModIdList.Count != 0)
                    {
                        ...
                        for (int index1 = 0; index1 < num1; ++index1)
                        {
                            ...
                            byte[] bytes = Encoding.ASCII.GetBytes(string.Format("key={0}&format=json&itemcount={1}{2}", (object)SteamUtils.SteamWebApiKey, (object)num2, (object)str));
        ...
        public static SteamUserDetailResponse GetSteamUserDetails(List<string> steamIdList)
        {
            ...
            try
            {
                ...
                for (int index1 = 0; index1 < num1; ++index1)
                {
                    ...
                    WebRequest webRequest = WebRequest.Create("https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key=" + SteamUtils.SteamWebApiKey + "&format=json&steamids=" + str1);
                    webRequest.Timeout = 30000;
        ...
        public static SteamCmdAppManifest ReadSteamCmdAppManifestFile(string file) => string.IsNullOrWhiteSpace(file) || !System.IO.File.Exists(file) ? (SteamCmdAppManifest)null : SteamCmdManifestDetailsResult.Deserialize(VdfDeserializer.FromFile(file).Deserialize());

        public static SteamCmdAppWorkshop ReadSteamCmdAppWorkshopFile(string file) => string.IsNullOrWhiteSpace(file) || !System.IO.File.Exists(file) ? (SteamCmdAppWorkshop)null : SteamCmdWorkshopDetailsResult.Deserialize(VdfDeserializer.FromFile(file).Deserialize());

        public static string SteamWebApiKey => !string.IsNullOrWhiteSpace(MainForm.Settings.SteamKey) ? MainForm.Settings.SteamKey : MainForm.Settings.DefaultSteamKey;

        public static Process GetSteamProcess()
        {
            //TODO:PlaceSettings
            string SteamClientFile = Path.Combine(MainForm.Settings.SteamCMDLocation, "steamcmd.exe");
